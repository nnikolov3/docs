...
- # **Assert Exceptions**: Use `pytest.raises` when testing for expected errors.Strict Bash Coding Standard
...
- **Self-Documenting Code**: Code must be self-documenting. This is achieved through meticulously chosen, intention-revealing names for variables and functions. Comments are used to explain _why_ a complex command is used, not _what_ it does.
